 ...
 *  101 TOKEN ERROR - DCToken inactive (needs activation)
 *  102 TOKEN ERROR - DCToken not found
 *  103 TOKEN ERROR - DCToken cancelled (marked as terminated)
 *  
...
public class DCVerificationResponse {
	...
	public String toString() {
		...
		try {
			JAXBContext ctx = JAXBContext.newInstance(DCVerificationResponse.class);
			Marshaller m = ctx.createMarshaller();
			StringWriter sw = new StringWriter();
			m.marshal(this, sw);
			result = sw.toString();
		} catch (Exception e) {
			log.warning("ADSS: No XML/String representation for DCVerificationResponse [" + this.hashCode() + "].");
		}
