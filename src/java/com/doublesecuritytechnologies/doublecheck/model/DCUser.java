...
/**
 * DCUser: Core class of the Data Model for the Double Security Service.
 * 
 * A DCUser represents a person that is a token holder. It is a user of some 
 * application within any of the service's clients that has a DCToken in their 
...
public class DCUser implements Serializable {
	...
	public DCUser() {
		super();
		log.finest("ADSS: DCUser (managed model bean) instantiated.");
	}
	
	public DCUser(String idfr, String name) {
		this();
		setIdfr(idfr);
		setName(name);
	}
	...
	public String toString() {
		...
		try {
			JAXBContext ctx = JAXBContext.newInstance(DCUser.class);
			Marshaller m = ctx.createMarshaller();
			StringWriter sw = new StringWriter();
			m.marshal(this, sw);
			result = sw.toString();
		} catch (Exception e) {
			log.warning("ADSS: No XML/String representation for DCUser [" + this.hashCode() + "].");
		}
